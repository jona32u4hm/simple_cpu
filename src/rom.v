...
module ROM(
 ...
 always @(*) begin
        case(PROG_ADDR)
            ...
            // STEP 0x18: ldr a, [0] 
            12'h018: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_LDR, 1'b0, 8'h00}; 

            // STEP 0x19: ldr b, [0] ; Load the SAME value into B
            12'h019: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_LDR, 1'b1, 8'h00};  

            // STEP 0x1A: sub b, a   ; b = b - a. Since a == b, b is now 0.
            12'h01A: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_SUB, 1'b1, 8'h00}; 

            // STEP 0x1B: ldr a, [4] ; Load any non-zero value into a
            12'h01B: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_LDR, 1'b0, 8'h04}; 

            // STEP 0x1C: and a, b   ; a = a & 0. Result MUST be 0, and EQUAL is set.
            12'h01C: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_AND, 1'b0, 8'h00}; 

            // STEP 0x1D: ldr a, [2] ; Load a known value 
            12'h01D: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_LDR, 1'b0, 8'h02}; 

            // STEP 0x1E: ior a, b   ; a = a | 0. Result should be the original value, zero flag reset
            12'h01E: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_IOR, 1'b0, 8'h00}; 

            // STEP 0x1F: ldr b, [2] ; Load the same value into b
            12'h01F: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_LDR, 1'b1, 8'h02}; 

            // STEP 0x20: and b, a  
            12'h020: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_AND, 1'b1, 8'h00}; 

            // STEP 23: cmp        ; zero flag set because registers are equal
            12'h021: {OPCODE, AB_SELECT, OP_ADDR} = {`CPU_CMP, 1'b0, 8'h00}; 
